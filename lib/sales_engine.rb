...
class SalesEngine
  attr_reader :merchants, :items, :invoices, :transactions, :invoice_items, :customers
  ...
  def self.from_csv(csv_file_locations)
    ...
    se.customers.from_csv(csv_file_locations[:customers]) if csv_file_locations[:customers]
    return se
  end
end
